...
import GamerOver from './components/GameOverView'
...
class App extends Component {
  state = {
    initialThumbnailUrl: imagesList[0].imageUrl,
    initialTabId: tabsList[0].tabId,
    score: 0,
    seconds: 60,
    isGameOver: false,
  }
  ...
  toggleTabId = tabId => {
    this.setState({initialTabId: tabId})
  }

  toggleImageUrl = image => {
    const {initialThumbnailUrl} = this.state
    const getIsEqual = imagesList.find(
      item => item.imageUrl === initialThumbnailUrl,
    )
    const {id} = getIsEqual
    if (id === image) {
      const randomImage =
      ...
      this.setState(prevState => ({
        initialThumbnailUrl: randomImage.imageUrl,
  ...
  renderTabItem = () => {
    const {initialTabId} = this.state
    return (
      <ul className="tab-item">
        {tabsList.map(tabItem => (
          <TabItem
            tabsList={tabItem}
            active={tabItem.tabId === initialTabId}
            toggleTabId={this.toggleTabId}
            key={tabItem.tabId}
          />
        ))}
      </ul>
  ...
  renderTabItemDetailsView = () => {
    const {initialTabId} = this.state
    const filteredImagesList = imagesList.filter(
      item => item.category === initialTabId,
    )
    return (
      <ul className="tab-item-details">
        {filteredImagesList.map(imageItem => (
          <TabItemDetails
            toggleImageUrl={this.toggleImageUrl}
  ...
  render() {
    const {score, seconds, isGameOver, initialThumbnailUrl} = this.state
    return (
      <div>
        ...
        <div className="bg-container">
          <div className="drop">
            {isGameOver ? (
              <GamerOver score={score} tryAgain={this.tryAgain} />
            ) : (
              <>
                <img
                  ...
                  alt="a"
                />
                {this.renderTabItem()} {this.renderTabItemDetailsView()}
              </>
            )}
          </div>
        </div>
